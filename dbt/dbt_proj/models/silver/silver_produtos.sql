...
SELECT
    ...
    -- Colunas calculadas
    CASE 
        WHEN preco_atual::numeric > 1000 THEN 'PREMIUM'
        WHEN preco_atual::numeric > 500 THEN 'MEDIO'
        ELSE 'BASICO'
    END AS faixa_preco,
    -- Validações
    CASE 
        WHEN preco_atual::numeric < 0 THEN TRUE
        ELSE FALSE
    END AS flag_preco_invalido
FROM {{ source('bronze', 'produtos_bronze') }}
